        ...
        <View style={styles.section}>
          ...
          {clientSearchResults.length > 0 && (
            <FlatList
              ...
              keyExtractor={(item, index) => item.id ? String(item.id) : `client-result-${index}`}
              renderItem={({ item }) => (
                <TouchableOpacity
        ...
        {orderLines.length > 0 && (
          <View style={styles.section}>
            ...
            {orderLines.map((line, index) => (
              <View key={`order-line-${line.fk_product}-${index}`} style={styles.orderLine}>
        ...
        <View style={styles.section}>
          ...
          >
            {shippingMethods.map((shipping, index) => (
              <TouchableOpacity
                key={`shipping-${shipping.id}-${index}`}
                style={[
                  styles.optionBox,
